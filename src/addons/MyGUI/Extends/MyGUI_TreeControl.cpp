			...
			if (nIndex >= (size_t)mnTopIndex)
			{
				...
				if (pIcon)
				{
					...
					if (pIconResource)
					{
                        ...
                        ImageIndexInfo IconInfo = pIconResource->getIndexInfo(strIconType, pNode->getImage());
						if (IconInfo.size.empty())
                            pIcon->setItemResourceInfo(pIconResource->getIndexInfo("Common", pNode->getImage()));
						else
