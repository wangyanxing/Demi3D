...
namespace micropather
{
    ...
    */
    ...
    void MicroPather::StatesInPool(micropather::MPVector< void* >* stateVec)
    {
        stateVec->clear();
    ...
    void PathNodePool::AllStates(unsigned frame, micropather::MPVector< void* >* stateVec)
    {
    ...
    int MicroPather::SolveForNearStates(void* startState, micropather::MPVector< StateCost >* nearstate, float maxCost)
    {
        ...
        for (PathNode* pNode = closedSentinel.next; pNode != &closedSentinel; pNode = pNode->next) {
            if (pNode->totalCost <= maxCost) {
                ...
                nearstate->push_back(sc);
            }
        }
#ifdef DEBUG
//         for (unsigned i = 0; i < nearstate->size(); ++i) {
//             for (unsigned k = i + 1; k < nearstate->size(); ++k) {
//                 MPASSERT((*nearstate)[i].state != (*nearstate)[k].state);
//             }
//         }
#endif
