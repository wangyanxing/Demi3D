...
namespace Demi 
{
    ...
    DiInStreamDataStream::~DiInStreamDataStream()
    {
        Close();
    }
    
    size_t DiInStreamDataStream::Read(void* buf, size_t count)
    {
        mInStream->read(static_cast<char*>(buf),
                        static_cast<std::streamsize>(count));
        return (size_t)mInStream->gcount();
    ...
    size_t DiInStreamDataStream::Write(const void* buf, size_t count)
    {
        DI_WARNING("Cannot write data to read-only data stream");
        return 0;
    }
    
    size_t DiInStreamDataStream::ReadLine(char* buf, size_t maxCount,
                                            const DiString& delim)
    {
        if (delim.empty())
        {
    ...
    void DiInStreamDataStream::Skip(long count)
    {
        //mInStream->clear();
        mInStream->seekg(static_cast<std::ifstream::pos_type>(count), std::ios::cur);
    }
    
    void DiInStreamDataStream::Seek( size_t pos )
    {
        //mInStream->clear();
        mInStream->seekg(static_cast<std::streamoff>(pos), std::ios::beg);
    }
    
    size_t DiInStreamDataStream::Tell(void) const
    {
        //mInStream->clear();
        return (size_t)mInStream->tellg();
    }
    
    bool DiInStreamDataStream::Eof(void) const
    {
        return mInStream->eof();
    }
    
    void DiInStreamDataStream::Close(void)
    {
        mInStream = nullptr;
    }
