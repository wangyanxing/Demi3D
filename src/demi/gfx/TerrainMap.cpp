...
namespace Demi
{
    ...
    bool DiTerrainMap::Load( const DiString& sceneName )
    {
        ...
        DiTerrainDesc& desc = scene->GetTerrainDesc();
        if (!desc.CheckValid())
        {
            DI_WARNING("Invalid terrain description file : %s",sceneName.c_str());
            return false;
        }
        ...
        {
            if (!mDesc->mLayers[i].DemiureId)
            {
                mDesc->mLayers[i].DemiureId = DI_NEW BYTE[gridSize];
                memset(mDesc->mLayers[i].DemiureId,0,gridSize*sizeof(BYTE));
            }
        }        
    ...
    DiMaterialPtr DiTerrainMap::GetMaterial(const DiPair<BYTE,BYTE>& textureid)
    {
        ...
        if(texTable.empty())
        {
            return DiAssetManager::GetInstance().GetAsset<DiMaterial>("unlighting_color.mtl");
        }
    ...
    {
        if (mDesc->DemiureTable.empty())
        {
            mDesc->DemiureTable[0] = texBaseName;
        }
        ...
        mDesc->DemiureTable[maxid+1] = texBaseName;
