...
// Update: 9/15/2003
//
// Added functions to create DI_NEW image from a buffer of pixels. Added function
...
{
    DI_ASSERT(format != 0);
    DI_ASSERT(components != 0);
    DI_ASSERT(baseImage.get_depth() == 1);
                                      ...
                                      const CTexture &positiveZ, const CTexture &negativeZ)
{
    DI_ASSERT(format != 0);
    DI_ASSERT(components != 0);
    DI_ASSERT(positiveX.get_depth() == 1);

    // verify that all dimensions are the same 
    DI_ASSERT(same_size(positiveX, negativeX));
    DI_ASSERT(same_size(positiveX, positiveY));
    DI_ASSERT(same_size(positiveX, negativeY));
    DI_ASSERT(same_size(positiveX, positiveZ));
    DI_ASSERT(same_size(positiveX, negativeZ));
...
bool CDDSImage::load(DiDataStream* fp, bool flipImage)
{
    DI_ASSERT(fp != 0);
    ...
    // read in DDS header
    DDS_HEADER ddsh;
    fp->Read(&ddsh,sizeof(DDS_HEADER));
...
{
    DI_ASSERT(get_num_mipmaps() == texture.get_num_mipmaps());
...
bool CDDSImage::save(DiString filename, bool flipImage)
{
    DI_ASSERT(m_valid);
...
{
    unsigned char *tmp = DI_NEW unsigned char[size];
    ...
    m_pixels(NULL)
{
    if (copy.get_size() != 0)
    {
        ...
        m_pixels = DI_NEW unsigned char[m_size];
...
void CSurface::create(unsigned int w, unsigned int h, unsigned int d, unsigned int imgsize, const unsigned char *pixels)
{
    ...
    m_depth = d;
    m_size = imgsize;
    m_pixels = DI_NEW unsigned char[imgsize];
