...
static tga_result tga_read_rle_stream(tga_image *dest, Demi::DiDataStream *fp);
...
tga_result tga_read_from_stream( tga_image *dest, DiDataStream *fp )
{
#define BARF(errcode) \
    { tga_free_buffers(dest);  return errcode; }

#define READ(destptr, size) \
    if(fp->Read(destptr,size) != (size_t)size)BARF(TGAERR_EOF)

#define READ16(dest) \
    { if(fp->Read(&dest,2) != 2)BARF(TGAERR_EOF);dest = letoh16(dest); }
    ...
    {
        /* read RLE */
        tga_result result = tga_read_rle_stream(dest, fp);
        if (result != TGA_NOERR) BARF(result);
    }
    else
    {
...
tga_result tga_read_rle_stream( tga_image *dest, Demi::DiDataStream *fp )
{
#define RLE_BIT BIT(7)
#define READ(dest, size) \
    if (fp->Read(dest,size) != (size_t)size) return TGAERR_EOF
    ...
    while ((p_loaded < p_expected) && !fp->Eof())
    {
        uint8_t b;
        READ(&b, 1);
        if (b & RLE_BIT)
