...
namespace MyGUI
{
    ...
    void DirectXRenderManager::doRender(IVertexBuffer* _buffer, ITexture* _texture, size_t _count)
    {
        ...
        DiD3D9Driver::StateCache->SetTexture(0, dxTex->getDirectXTexture());
    ...
    {
        DiD3D9StateCache* cache = DiD3D9Driver::StateCache;
        cache->SetRenderState(D3DRS_ALPHABLENDENABLE, TRUE);

        cache->SetTextureStageState(0, D3DTSS_COLOROP, D3DTOP_MODULATE);
        cache->SetTextureStageState(0, D3DTSS_COLORARG0, D3DTA_DIFFUSE);
        cache->SetTextureStageState(0, D3DTSS_COLORARG1, D3DTA_TEXTURE);
        cache->SetTextureStageState(0, D3DTSS_COLORARG2, D3DTA_DIFFUSE);

        cache->SetTextureStageState(0, D3DTSS_ALPHAOP, D3DTOP_MODULATE);
        cache->SetTextureStageState(0, D3DTSS_ALPHAARG0, D3DTA_DIFFUSE);
        cache->SetTextureStageState(0, D3DTSS_ALPHAARG1, D3DTA_TEXTURE);
        cache->SetTextureStageState(0, D3DTSS_ALPHAARG2, D3DTA_DIFFUSE);

        cache->SetSamplerState(0, D3DSAMP_MINFILTER, D3DTEXF_LINEAR);
        cache->SetSamplerState(0, D3DSAMP_MAGFILTER, D3DTEXF_LINEAR);
        cache->SetSamplerState(0, D3DSAMP_MIPFILTER, D3DTEXF_NONE);

        cache->SetSamplerState(0, D3DSAMP_ADDRESSU, D3DTADDRESS_CLAMP);
        cache->SetSamplerState(0, D3DSAMP_ADDRESSV, D3DTADDRESS_CLAMP);

        cache->SetRenderState(D3DRS_SRCBLEND, D3DBLEND_SRCALPHA);
        cache->SetRenderState(D3DRS_DESTBLEND, D3DBLEND_INVSRCALPHA);

        cache->SetRenderState(D3DRS_ZWRITEENABLE, FALSE);
        cache->SetRenderState(D3DRS_ZENABLE, FALSE);
        cache->SetRenderState(D3DRS_LIGHTING, 0);
        cache->SetRenderState(D3DRS_CULLMODE, D3DCULL_NONE);

        cache->SetRenderState(D3DRS_ALPHATESTENABLE, FALSE);
